    ...
    public static int[][] generateMap(int width, int height) {
        ...
        for (int i = 0; i < width/10; i++) {
            for (int j = 0; j < height/10; j++) {
                double numberNoise = noise(i, j);
                numberNoise += 1;
                if (numberNoise < 0.103)
                    map[i][j] = 0;
                else if (numberNoise <0.1)
                    map[i][j] = 1;
                else if (numberNoise < 0)
                    map[i][j] = 2;
                else
                    map[i][j] = 3;

                //TreeManager.getInstance().createTree(new Vector2i(i, j));
